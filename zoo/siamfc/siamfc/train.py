...
from .custom_transforms import ToTensor, RandomStretch, \
...
def train(gpu_id, data_dir):
    ...
    train_loader = DataLoader(train_dataset, batch_size=config.train_batch_size,
                              shuffle=True, pin_memory=True, num_workers=config.train_num_workers,
                              drop_last=True)
    valid_loader = DataLoader(valid_dataset, batch_size=config.valid_batch_size,
                              shuffle=False, pin_memory=True, num_workers=config.valid_num_workers,
                              drop_last=True)
    ...
    with torch.cuda.device(gpu_id):
        ...
        for epoch in range(config.epoch):
            ...
            for i, data in enumerate(tqdm(train_loader)):
                exemplar_img, instance_img = data
                exemplar_var, instance_var = Variable(exemplar_img.cuda()), Variable(instance_img.cuda())
                optimizer.zero_grad()
            ...
            train_loss = torch.mean(torch.stack(train_loss)).cpu().detach().numpy()

            valid_loss = []
            model.eval()
            for i, data in enumerate(tqdm(valid_loader)):
                exemplar_img, instance_img = data
                exemplar_var, instance_var = Variable(exemplar_img.cuda()),\
                                             Variable(instance_img.cuda())
                outputs = model((exemplar_var, instance_var))
                loss = model.weighted_loss(outputs)
                valid_loss.append(loss.data)
#            valid_loss = np.mean(valid_loss)
            valid_loss = torch.mean(torch.stack(valid_loss)).cpu().detach().numpy()
            print("EPOCH %d valid_loss: %.4f, train_loss: %.4f" %
                    (epoch, valid_loss, train_loss))
            summary_writer.add_scalar('valid/loss', 
                    valid_loss, (epoch+1)*len(train_loader))
