...
def is_fake_bn(m):
    from model.resnet import Lambda
    return isinstance(m, Lambda)


def search_absorb_fake_bn(model, prev=None, remove_bn=True, verbose=False):
    with torch.no_grad():
        for m in model.children():
            if is_fake_bn(m) and is_absorbing(prev):
                # print(prev,m)
                absorb_fake_bn(prev, m, verbose=verbose)
            search_absorb_fake_bn(m, remove_bn=remove_bn, verbose=verbose)
            prev = m
